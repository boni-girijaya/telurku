...
let db = loadData();
let state = { role: "owner", page: "dashboard", reportDate: today, gradingDate: today, directDate: today, loadedDate: today, selectedReport: null, selectedTrip: null, editReportId: null, editCageId: null, editUserId: null, editDriverId: null, showUserForm: false, showDriverForm: false };
...
function assignedCages(userName = currentUser().name) {
  if (state.role === "kandang" && authState.profile?.id) {
    return db.cages.filter(c => (c.keeperIds || []).includes(authState.profile.id));
  }
...
function refreshUserAssignmentsFromCages() {
  ...
  db.users = db.users.map(user => {
    ...
    const assigned = db.cages.filter(c => (c.keeperIds || []).includes(user.id)).map(c => c.name);
...
async function loadOperationalData(targetDate = activeDataDate()) {
  const [{ data: profiles }, { data: cages, error: cageError }, { data: assignments, error: assignmentError }] = await Promise.all([
    ...
    supabase.from("cage_assignments").select("cage_id, profile_id"),
  ]);
  if (cageError) throw cageError;
  if (assignmentError) throw assignmentError;

  const profileMap = new Map((profiles || []).map(profile => [profile.id, profile]));
  const assignmentsByCage = new Map();
  for (const item of assignments || []) {
    const list = assignmentsByCage.get(item.cage_id) || [];
    list.push(item.profile_id);
    assignmentsByCage.set(item.cage_id, list);
  }
  db.cages = (cages || []).map(c => ({
    ...
    keeperIds: assignmentsByCage.get(c.id) || (c.keeper_id ? [c.keeper_id] : []),
    keeper: (assignmentsByCage.get(c.id) || (c.keeper_id ? [c.keeper_id] : []))
...
function directWeightPage() {
  ...
  <div class="form-card"><div class="card card-pad">
    ...
    <form id="direct-weight-form" style="margin-top:22px">
      ...
      <div class="form-row"><div class="form-group"><label>Supir</label><select name="driver" required><option value="">Pilih supir</option>${db.drivers.map(d=>`<option>${escapeHtml(d)}</option>`).join("")}</select></div><div class="form-group"><label>Berat bersih (kg)</label><input name="weight" type="number" min="0.01" step="0.01" required /></div></div>
...
function settingsPage() {
  ...
  return `<div class="section-head"><div><h2>Kontrol koreksi setoran</h2><p>Owner dapat membuka atau mengunci tombol ubah dan batalkan setoran.</p></div></div>
    <div class="card card-pad">
      ...
      <div class="hint" style="margin:18px 0">Saat aktif, tombol Ubah dan Batalkan hanya muncul untuk Admin dan Owner. Anak Kandang dan Kepala Penerimaan tetap tidak bisa mengoreksi data lama.</div>
...
function bindEvents() {
  ...
  document.querySelector("#direct-weight-form")?.addEventListener("submit",async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);const values={date:state.directDate,cageId:Number(fd.get("cageId")),trip:Number(fd.get("trip")),driver:String(fd.get("driver")),weight:Number(fd.get("weight")),note:String(fd.get("note")||"").trim()};try{const reference=await saveDirectWeightToSupabase(values);await loadOperationalData(state.directDate);audit(`input kg langsung ${reference} sebesar ${fmtKg(values.weight)} kg untuk ${state.directDate}`);render();toast("Input kg langsung tersimpan")}catch(error){toast(error.message||"Gagal menyimpan kg langsung")}});
...
async function saveCageToSupabase(cageId, values) {
  const oldKeeperIds = cage(cageId)?.keeperIds || [];
  const keeperIds = values.keeperIds || [];
  const { error } = await supabase
    .from("cages")
    .update({
      name: values.name,
      ...
      note: values.note,
      keeper_id: keeperIds[0] || null,
      updated_at: new Date().toISOString(),
    })
    .eq("id", cageId);
  if (error) throw error;
  const { error: deleteError } = await supabase.from("cage_assignments").delete().eq("cage_id", cageId);
  if (deleteError) throw deleteError;
  if (keeperIds.length) {
    const { error: insertError } = await supabase.from("cage_assignments").insert(
      keeperIds.map(profileId => ({ cage_id: cageId, profile_id: profileId }))
    );
    if (insertError) throw insertError;
  }
  const affected = [...new Set([...oldKeeperIds, ...keeperIds])];
  for (const profileId of affected) await updateKeeperAssignmentSummary(profileId);
}
...
async function updateKeeperAssignmentSummary(profileId) {
  ...
  const { data, error } = await supabase
    ...
    .select("cages(id, name)")
    .eq("profile_id", profileId);
  if (error) throw error;
  const assignment = data?.length
    ? data.map(row => row.cages).filter(Boolean).sort((a,b) => a.id - b.id).map(c => c.name).join(", ")
    : "Belum ditentukan";
